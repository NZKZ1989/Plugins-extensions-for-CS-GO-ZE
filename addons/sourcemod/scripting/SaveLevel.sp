...
public void OnClientPostAdminCheck(int client)
{
	...
	if(g_PlayerLevels.GetString(sSteamID, sTargets, sizeof(sTargets)))
	{
		...
		if(Found)
			PrintToChatAll(" \x01\x0B\x03[SaveLevel]\x01 \x04%N\x01 has been restored to: \x04%s", client, sNames);
	}
}
...
public Action Command_Level(int client, int args)
{
	...
	if(sPrevNames[0])
		ShowActivity2(client, " \x01\x0B\x03[SaveLevel]\x01 ", "Set \x04%s\x01 from \x04%s\x01 to \x04%s\x01", sTargetName, sPrevNames, sName);
	else
		ShowActivity2(client, " \x01\x0B\x03[SaveLevel]\x01 ", "Set \x04%s\x01 to \x04%s\x01", sTargetName, sName);
