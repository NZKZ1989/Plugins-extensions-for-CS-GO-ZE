...
{
    name = "Gloves",
    author = "kgns | oyunhost.net | adaptation for ZR",
    description = "CS:GO Gloves Management (Zombie Reloaded support)",
    version = "1.0.5-zr",
...
public Action CommandGlove(int client, int args)
{
    if (IsValidClient(client))
    {
        ...
        {
            PrintToChat(client, "[Gloves] Зомби не могут использовать перчатки!");
...
public void ZR_OnClientInfected(int client, int attacker)
{
    if (IsValidClient(client))
    {
        ResetPlayerGloves(client);
    }
}

public void APIOnClientHuman(int client, bool respawn, bool protect)
{
    if (IsValidClient(client))
    {
...
public void APIOnClientHumanPost(int client, bool respawn, bool protect)
{
    if (IsValidClient(client))
    {
        GivePlayerGloves(client);
...
public void GivePlayerGloves(int client)
{
    ...
    if (g_iGloves[client][team] != 0)
    {
        ...
        if (ent != -1)
        {
            ...
            if (g_iGloves[client][team] == -1)
            {
                char buffer[20];
                char buffers[2][10];
                GetRandomSkin(client, team, buffer, sizeof(buffer), g_iGroup[client][team]);
            ...
            {
                SetEntProp(ent, Prop_Send, "m_iItemDefinitionIndex", g_iGroup[client][team]);
                SetEntProp(ent, Prop_Send, "m_nFallbackPaintKit", g_iGloves[client][team]);
            }

            SetEntPropFloat(ent, Prop_Send, "m_flFallbackWear", g_fFloatValue[client][team]);
...
void ResetPlayerGloves(int client)
{
