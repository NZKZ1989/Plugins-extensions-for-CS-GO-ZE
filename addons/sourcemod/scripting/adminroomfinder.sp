...
stock bool logicalButtonMatch(int entity)
{
    ...
    GetEntPropString(entity, Prop_Data, "m_iParent", buffer, sizeof(buffer));
    if (strlen(buffer))
    {
        // BAD MATCH
        return false;
    ...
    // Проверка output'ов через новый API
    int count = GetOutputCount(entity, "m_OnPressed");
    for (int output = 0; output < count; output++)
    {
        GetOutputParameter(entity, "m_OnPressed", output, buffer, sizeof(buffer));
