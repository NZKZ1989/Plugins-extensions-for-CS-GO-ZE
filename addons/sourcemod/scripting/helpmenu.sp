...
public Plugin myinfo = {
	...
	author = "chundo, emsit, joinedsenses,donut1k and fixed by FreedomZ",
...
public void OnPluginStart() {
	...
	RegConsoleCmd("sm_menu", Command_HelpMenu, "Display the help menu.");
...
public Action Timer_WelcomeMessage(Handle timer, int userid) {
	...
	if (client && IsClientInGame(client) && !IsFakeClient(client)) {
		PrintToChat(client, "\x05 \x01Для помощи по серверу используйте команду \x04!helpmenu\x01 в чат");
	}
}
...
public Action Command_HelpMenuReload(int client, int args) {
	...
	PrintToChat(client, "\x05[SM] \x01Конфигурационный файл был перезагружен");
...
void Help_ShowMainMenu(int client) {
	...
	menu.SetTitle("Меню помощи по серверу");
	...
	if (g_cvarRotation.BoolValue) {
		menu.AddItem("maplist", "Карты сервера");
	}
	if (g_cvarAdmins.BoolValue) {
		menu.AddItem("admins", "Лист онлайн администраторов");
	}
	...
	switch(action) {
		case MenuAction_Select: {
			...
			if (param2 == msize) { // Maps
				...
				Format(buf, sizeof(buf), "Карты сервера (%d maps)\n ", g_helpMenus.Length);
				...
				adminMenu.SetTitle("Онлайн Администраторы");
				char aname[64];
				...
				if (param2 <= msize) {
					...
					if (hmenu.type == HelpMenuType_Text) {
						...
						cpanel.DrawText(" ");
						cpanel.DrawItem("Назад", ITEMDRAW_CONTROL);
						cpanel.DrawItem(" ", ITEMDRAW_NOTEXT);
						cpanel.DrawText(" ");
						cpanel.DrawItem("Выход", ITEMDRAW_CONTROL);
						hmenu.items.Reset();
		...
		case MenuAction_Select: {
			...
			if (strlen(itemval) > 0) {
				...
				if (CommandExist(command, sizeof(command)) || CommandExist(itemval, sizeof(itemval))) {
					...
					panel.DrawText(" ");
					panel.DrawItem("Назад", ITEMDRAW_CONTROL);
					panel.DrawItem(" ", ITEMDRAW_NOTEXT);
					panel.DrawText(" ");
					panel.DrawItem("Выход", ITEMDRAW_CONTROL);
