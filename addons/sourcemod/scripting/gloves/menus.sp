    ...
    switch (action)
    {
        case MenuAction_Select:
        {
            if (IsClientInGame(client))
            {
                ...
                // Проверка: зомби не могут использовать меню
                ...
                if (index == 0 || index == -1)
                {
                    ...
                    // Всегда используем только CT
                    Format(updateFields, sizeof(updateFields), "ct_group = %d, ct_glove = %d", index, index);
                    UpdatePlayerData(client, updateFields);
...
public int MainMenuHandler(Menu menu, MenuAction action, int client, int selection)
{
    switch (action)
    {
        case MenuAction_Select:
        {
            if (IsClientInGame(client))
            {
                // Зомби не могут использовать меню
                if (ZR_IsClientZombie(client))
                {
                    PrintToChat(client, "[Gloves] Зомби не могут использовать перчатки!");
                    return 0;
                ...
                else if (StrEqual(info, "ct"))
                {
                    ...
                    menuGlovesGroup[g_iClientLanguage[client]][CS_TEAM_CT].Display(client, MENU_TIME_FOREVER);
                }
            }
        }
...
Menu CreateMainMenu(int client)
{
    ...
    // Проверка: зомби не могут открывать меню
    if (ZR_IsClientZombie(client))
    {
        PrintToChat(client, "[Gloves] Зомби не могут использовать перчатки!");
        return menu;
    ...
    // Добавляем пункт "Float" только для Human
    if (g_iEnableFloat == 1 && IsPlayerAlive(client))
    {
        int playerTeam = GetClientTeam(client);
        if (playerTeam == CS_TEAM_CT && g_iGloves[client][playerTeam] != 0)
        {
            g_iTeam[client] = playerTeam;
            float fValue = g_fFloatValue[client][playerTeam] * 100.0;
            int wear = 100 - RoundFloat(fValue);
