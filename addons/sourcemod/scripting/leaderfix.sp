...
public void OnPluginStart()
{
	...
	g_cVDefendVMT = CreateConVar("sm_leader_defend_vmt", "materials/sg/defend_trg.vmt", "The defend here .vmt file");
	g_cVDefendVTF = CreateConVar("sm_leader_defend_vtf", "materials/sg/defend_trg.vtf", "The defend here .vtf file");
	g_cVFollowVMT = CreateConVar("sm_leader_follow_vmt", "materials/sg/follow_trg.vmt", "The follow me .vmt file");
	g_cVFollowVTF = CreateConVar("sm_leader_follow_vtf", "materials/sg/follow_trg.vtf", "The follow me .vtf file");
	...
	if(CheckCommandAccess(client, "sm_admin", ADMFLAG_GENERIC, false))
	{
		if(args == 1)
		{
			...
			else
			{
				...
				{
					SetLeader(target);
					PrintToChatAll("[SM] %N теперь лидер!", target);
...
public void LeaderMenu(int client)
{
	...
	SetMenuTitle(menu, "Меню Лидера\nСпрайт: %s\nМаркер: %s\nМаяк: %s", sprite, marker, beacon);
	AddMenuItem(menu, "resign", "Отказаться от лидерства");
	AddMenuItem(menu, "sprite", "Меню спрайтов");
	AddMenuItem(menu, "marker", "Меню маркеров");
	AddMenuItem(menu, "beacon", "Переключить маяк");

	SetMenuExitButton(menu, true);
	DisplayMenu(menu, client, MENU_TIME_FOREVER);
}

public void LeaderMenu_Handler(Handle menu, MenuAction action, int client, int position)
{
	if(leaderClient == client && IsValidClient(client))
	{
		if(action == MenuAction_Select)
		{
			...
			{
				RemoveLeader(client);
				PrintToChatAll("[SM] %N отказался от лидерства!", client);
			}
...
public void SpriteMenu_Handler(Handle menu, MenuAction action, int client, int position)
{
    if(leaderClient == client && IsValidClient(client))
    {
        if(action == MenuAction_Select)
        {
            ...
            {
                RemoveSprite(client);
                PrintToChat(client, "[SM] Спрайт удалён.");
                currentSprite = -1;
                LeaderMenu(client);
            }
            if(StrEqual(info, "defend"))
            {
                ...
                PrintToChat(client, "[SM] Спрайт изменён на 'Защищать здесь'.");
                currentSprite = 0;
                LeaderMenu(client);
            }
            if(StrEqual(info, "follow"))
            {
                ...
                PrintToChat(client, "[SM] Спрайт изменён на 'Следуй за мной'.");
                currentSprite = 1;
                LeaderMenu(client);
            }
        }
...
public void MarkerMenu_Handler(Handle menu, MenuAction action, int client, int position)
{
    if(leaderClient == client && IsValidClient(client))
    {
        if(action == MenuAction_Select)
        {
            ...
            {
                RemoveMarker(client);
                PrintToChat(client, "[SM] Маркер удалён.");
                markerActive = false;
                LeaderMenu(client);
            }
            if(StrEqual(info, "defendmarker"))
            {
                ...
                PrintToChat(client, "[SM] Маркер 'Защищать здесь' установлен.");
                markerActive = true;
                LeaderMenu(client);
            }
        }
...
public void OnClientDisconnect(int client)
{
	if(client == leaderClient)
	{
		PrintToChatAll("[SM] Лидер отключился!");
		RemoveLeader(client);
	}
	voteCount[client] = 0;
}
...
public void ZR_OnClientInfected(int client, int attacker, bool motherInfect, bool respawnOverride, bool respawn)
{
    if(client == leaderClient)
    {
        PrintToChatAll("[SM] Лидер был заражён!");
        RemoveLeader(client);
    }
}
...
public void Native_SetLeader(Handle plugin, int numParams)
{
...
public Action Radio(int client, const char[] command, int argc)
{
	if(client == leaderClient)
	{
		if(StrEqual(command, "compliment")) PrintRadio(client, "Отлично!");
